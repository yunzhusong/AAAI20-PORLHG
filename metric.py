...
@curry
#def compute_rouge_l(output, reference, TEXT, vocab_size, word_embeddings, mode='f'):
def compute_rouge_l(output, reference, mode='f'):
    ...
    gen_score = 0
    #if vocab_size != None:
        ## method 1: the classifier result
        #gen_score = do_inference(output, TEXT, vocab_size, word_embeddings)

        ## method 2: the cover rate with comment mtx
    ...
    if lcs == 0:
        ...
        if mode == 'p':
            score = weight_f*precision + weight_p*gen_score
        if mode == 'r': 
            score = weight_f*recall + weight_p*gen_score
        else:
            score = weight_f*f_score + weight_p*gen_score
    return score
