...
class RLDataset(CnnDmDataset):
    ...
    def __getitem__(self, i):
        ...
        abs_sents = js_data['headline']
        return art_sents, topic, abs_sents
...
def train(args):
    ...
    if args.load_ckpt == True:
        rl_dir = args.path
        rl_ckpt, trained_step = load_best_ckpt(rl_dir, reverse=True)
        agent.load_state_dict(rl_ckpt)
    ...
    trainer = BasicTrainer(pipeline, args.path,
                           ...
                           val_mode='score', trained_step=trained_step)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr', type=float, action='store', default=1e-5,
                        help='learning rate')
    ...
    parser.add_argument('--load_ckpt', type=bool, action='store', default=False)
