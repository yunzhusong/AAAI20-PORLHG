...
def coll_fn_extract(data):
    def is_good_data(d):
        ...
        source_sents, topics, extracts = d
...
@curry
def tokenize_decode(max_len, data):
    texts, topics = data
    return [t.lower().split()[:max_len] for t in texts], topics
...
def convert_batch_extract_ptr(unk, word2id, batch):
    def convert_one(sample):
        ...
        return id_sents, topics, extracts
    batch = list(map(convert_one, batch))
    return batch, word2id
...
def pad_batch_tensorize(inputs, pad, cuda=True):
    ...
    #print("tensor_type:{}".format(tensor_type))
    #print("batch_size:{}".format(batch_size))
    #print("max_len:{}".format(max_len))
    #print("tensor_shape:{}".format(tensor_shape))
    #print("inputs:{}".format(inputs))

    #    print("there is an empty batcher!")
...
def batchify_fn_copy(pad, start, end, data, cuda=True):
    sources, ext_srcs, tar_ins, targets = tuple(map(list, unzip(data[0])))
    ext_word2id = data[1] # data is a tuple
    src_lens = [len(src) for src in sources]
    ...
    loss_args = (target, )
    return fw_args, loss_args, ext_word2id


@curry
def batchify_fn_extract_ptr(pad, data, cuda=True):
    source_lists, topics, targets = tuple(map(list, unzip(data[0])))
    ext_word2id = data[1] ## data is a tuple, for extractor the ext_word2id is just word2id 
    ...
    tar_in = pad_batch_tensorize(
        ...
        pad=-0, cuda=cuda # use 0 here for feeding first conv sentence repr. ## YUNZHU change pad=-0 to pad=0
    )

    fw_args = (sources, src_nums, tar_in, topics)
    loss_args = (target, )
    return fw_args, loss_args, ext_word2id

@curry
def batchify_fn_extract_ff(pad, data, cuda=True):
    source_lists, targets = tuple(map(list, unzip(data[0])))
    ext_word2id = data[1] ## data is a tuple
...
class BucketedGenerater(object):
    ...
    def __call__(self, batch_size: int):
        def get_batches(hyper_batch):
            ...
            for i in indexes:
                #batch, ext_word2id = self._batchify(hyper_batch[i:i+batch_size])
                fw_args, bw_args, ext_word2id = self._batchify(hyper_batch[i:i+batch_size])
                #yield batch, ext_word2id
