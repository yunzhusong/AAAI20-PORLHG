...
def compute_loss(net, criterion, fw_args, loss_args, ext_word2id):
    ...
    loss = criterion(*((net(*fw_args),) + loss_args), sent_topics)
    return loss

@curry
def val_step(loss_step, fw_args, loss_args, ext_word2id):
    loss, _, _ = loss_step(fw_args, loss_args, None)
...
class BasicPipeline(object):
    ...
    def train_step(self):
        ...
        if self.name.split('_')[-1] == 'extractor':
            ...
            loss, loss_topic, loss_xent = self._criterion(*loss_args, sent_topics) # for extractor
            loss = loss.mean()
            log_dict['loss_topic'] = loss_topic.mean().item()
            log_dict['loss_xent']  = loss_xent.mean().item()
        else:  
