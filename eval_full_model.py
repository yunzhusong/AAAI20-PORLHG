...
def main(args):
    ...
    #split = 'val'
    ref_path = "/data1/home2/Headline/Dataset/CNNDM/Headline_ref"
    ref_dir = join(ref_path, 'refs', split)
    #ref_dir = join(_DATA_DIR, 'refs', split)
    assert exists(ref_dir)
    #ref_dir = join(_DATA_DIR, 'refs')
    #assert exists(ref_dir)

    if args.rouge:
        dec_pattern = r'(\d+).dec'
        ref_pattern = '#ID#.ref'
        print("dec_dir: {}, ref_dir: {}".format(dec_dir, ref_dir))
