...
from model.util_ import sequence_loss_ext
from training_ext import get_basic_grad_fn, basic_validate
from training_ext import BasicPipeline, BasicTrainer
...
class ExtractDataset(CnnDmDataset):
    ...
    def __getitem__(self, i):
        ...
        art_sents, extracts = js_data['article'], js_data['extracted_headline']
        topic = js_data['topic_method']
        return art_sents, topic, extracts
...
def build_batchers(net_type, word2id, cuda, debug):
    ...
    batchify_fn = (batchify_fn_extract_ff if net_type == 'ff' # net_type is rnn
...
def configure_training(net_type, opt, lr, clip_grad, lr_decay, batch_size, word2id):
    ...
    if net_type == 'ff':
        ...
        #def criterion(logits, targets, ext_word2id):
        def criterion(logits, targets, sent_topics):
            return sequence_loss_ext(logits, targets, sent_topics, ce, pad_idx=-1)
            #return sequence_loss_ext(logits, targets, False, ce, pad_idx=-1)
...
def main(args):
    ...
    criterion, train_params = configure_training(
        args.net_type, 'adam', args.lr, args.clip, args.decay, args.batch, dict(word2id)
    )
