...
from .util import reconstruct_topic_dis
...
class ActorCritic(nn.Module):
    ...
    def __init__(self, sent_encoder, art_encoder,
        ...
        AP = False
        if AP == True:
            print("Loading comment mtx file...")
            comment_mtx = pkl.load(open('comment_mtx_fre.pkl', 'rb'))  ## comment_mtx.pkl
            self.weight = torch.FloatTensor(comment_mtx).cuda()
            word_list = []
            for i in range(30004):
                word_list.append(i)
            self.word_list = torch.LongTensor(word_list).cuda()
            #self.enc_pop_sent = self._sent_enc(word_list.unsqueeze(0), weight)
            #self.enc_pop = self._art_enc(enc_pop_sent.unsqueeze(0)).squeeze(0)

            #self.enc_sent = self._sent_enc(self.word_list, weight).unsqueeze(0)
            #self._enc_pop = self._art_enc(enc_sent).squeeze(0)
    ...
    def forward(self, raw_article_sents, topic, n_abs=None):
        article_sent = self._batcher(raw_article_sents)
        enc_sent = self._sent_enc(article_sent).unsqueeze(0) # ([1,18,300])
        enc_art = self._art_enc(enc_sent).squeeze(0) # ([18,512])

        topic = reconstruct_topic_dis(topic, self._topic_num)
        #topic = self._softmax(topic*1e1) # For balance the value

        #topic = torch.FloatTensor(topic).cuda()
        if len(topic.size()) == 1:
            topic = topic.unsqueeze(0)

        enc_art = torch.cat((enc_art, topic), dim=1)
