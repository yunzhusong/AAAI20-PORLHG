...
class DecodeDataset(CnnDmDataset):
    ...
    def __getitem__(self, i):
        ...
        topic = js_data['topic_method2']
        return art_sents, topic
...
def load_best_ckpt(model_dir, reverse=False):
    ...
    return ckpt, int(ckpts[0].split('-')[-1])
...
class Extractor(object):
    def __init__(self, ext_dir, max_ext=5, cuda=True):
        ...
        if ext_meta['net'] == 'ml_ff_extractor':
            ...
            raise ValueError()
        ext_ckpt, _ = load_best_ckpt(ext_dir)
    ...
    def __call__(self, raw_article_sents, topics):
        self._net.eval()
        n_art = len(raw_article_sents)
        articles = conver2id(UNK, self._word2id, raw_article_sents)
        article = pad_batch_tensorize(articles, PAD, cuda=False
                                     ).to(self._device)
        indices = self._net.extract([article], topics, k=min(n_art, self._max_ext))
        return indices
...
class RLExtractor(object):
    def __init__(self, ext_dir, cuda=True):
        ...
        ext_ckpt,_ = load_best_ckpt(ext_dir, reverse=True)
    ...
    def __call__(self, raw_article_sents, topic):
        self._net.eval()
        indices = self._net(raw_article_sents, topic)
        return indices
