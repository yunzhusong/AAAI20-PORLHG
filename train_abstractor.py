...
from data.batcher import BucketedGenerater, get_ext_word2id
...
from pop_classifier import classifier
...
class MatchDataset(CnnDmDataset):
    ...
    def __getitem__(self, i):
        ...
        art_sents, abs_sents, extracts = (
            js_data['article'], js_data['headline'], js_data['extracted_headline'])
        matched_arts = [art_sents[i] for i in extracts]
        #print("headline for {}: {}".format(i, abs_sents))
...
def configure_training(opt, lr, clip_grad, lr_decay, batch_size, word2id):
    ...
    nll = lambda logit, target : F.nll_loss(logit, target, reduce=None)
    def criterion(logits, cls_pro, targets, word2id):
        return sequence_loss(logits, cls_pro, targets, word2id, True, nll, pad_idx=PAD)
...
def main(args):
	...
	#ext_wors2id = get_ext_word2id(unk)
    #pdb.set_trace()
    # make net
    ...
    criterion, train_params = configure_training(
        'adam', args.lr, args.clip, args.decay, args.batch, dict(word2id)
    )
    ...
    with open(join(args.path, 'vocab.pkl'), 'rb') as f:
        wc_ = pkl.load(f)
    #pdb.set_trace()
    meta = {}
    ...
    # prepare trainer
    #val_fn = basic_validate(net, criterion, word2id)
